...
function Login({ setLoginUser }) {
    ...
    const login = () => {
        axios.post("https://lystclone.herokuapp.com/login", user)
            .then(res => {
    ...
    return (
        <div className="login">
            <div className="divone">
                <span><button onClick={()=> {history("/register")}} className="buttononee buttonwhite">New to Lyst?</button></span><span><button className="buttononee buttonblack">Already a member</button></span>
                <h2>Sign in</h2>
                <div className="labell"> 
                    <label className="labeel" htmlFor="">Email address</label>
                    <input type="text" name="email" value={user.email} onChange={handleChange} placeholder="Enter your email address"></input>
                    <label className="labeel" htmlFor="">Password</label>
